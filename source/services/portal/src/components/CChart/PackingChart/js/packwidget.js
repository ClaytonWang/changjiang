 ...
 */
PackWidget.prototype.createOptions = function (
  bins,
  defaultOptions,
  userOptions
) {
...
PackWidget.prototype.createThreeLights = function (options) {
  ...
  dirLight.position.set(1.5, 1.75, 1);
...
function createGroundPlane(bin) {
  ...
  for (var i = 0; i < bin.shape.length; i++) {
    if (bin.shape[i].y === minY) groundPoints.push(bin.shape[i]);
  }
  var xGroundPoints = groundPoints.map(function (point) {
    return point.x;
  });
...
PackWidget.prototype.createTempBins = function (bins) {
  ...
  for (var i = 0; i < bins.length; i++) {
    if (bins[i].shape === undefined) binMeshs.push(createTHREEBin(bins[i]));
    else {
...
PackWidget.prototype.createThreeItems = function (items) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    var line = new THREE.LineSegments(
      edge,
      new THREE.LineBasicMaterial({ color: items[i].borderColor })
    );
...
PackWidget.prototype.createThreeScene = function (bins, items, camera, lights) {
  ...
  for (var i = 0; i < bins.length; i++) scene.add(bins[i]);
  for (var i = 0; i < items.length; i++) scene.add(items[i]);
  scene.add(camera);
...
function createShapedTHREEBin(bin) {
  ...
  for (var j = 0; j < bin.shape.length; j++) {
    shape.moveTo(bin.shape[j].x, bin.shape[j].y);
  }
...
function createTHREEBin(bin) {
  var geometry = new THREE.BoxGeometry(bin.width, bin.height, bin.depth);
...
PackWidget.prototype.create = function (container) {
  ...
  this.options = this.createOptions(
    this.bins,
    this.createDefaultOptions(this.bins),
    this.userOptions
  );
  ...
  this.lights = lights;
  var scene = this.createThreeScene(
    graphicsBins,
    graphicsItems,
    camera,
    lights
  ...
  for (var i = 0; i < graphicsItems.length; i++) {
    domEvents.bind(
      ...
      "click",
      function (item) {
        itemClicked(scene.userData.itemMap.get(item.target));
      },
      false
    );
    ...
    domEvents.bind(
      ...
      "mouseover",
      function (item) {
        itemOver(scene.userData.itemMap.get(item.target));
      },
      false
    );
    ...
    domEvents.bind(
      ...
      "mouseout",
      function (item) {
        itemOut(scene.userData.itemMap.get(item.target));
      },
      false
    );
  }

  scene.userData.controls = new THREE.OrbitControls(
    scene.userData.camera,
    scene.userData.element
  );
  scene.userData.controls.minDistance = 1000;
  scene.userData.controls.maxDistance = 4500;
  ...
  scene.userData.controls.maxPolarAngle = Math.PI / 2;
  // lock Rotation if options is set
  if (this.options.verticalRotation === false) {
    scene.userData.controls.maxAzimuthAngle =
      scene.userData.controls.getAzimuthAngle();
    scene.userData.controls.minAzimuthAngle =
      scene.userData.controls.getAzimuthAngle();
  }
  if (this.options.horizontalRotation === false) {
    scene.userData.controls.maxPolarAngle =
      scene.userData.controls.getPolarAngle();
    scene.userData.controls.minPolarAngle =
      scene.userData.controls.getPolarAngle();
  }

  scene.userData.element.addEventListener(
    "mousemove",
    onDocumentMouseMove,
    false
  );
...
function PackWidgetRenderer() {
  ...
  function render() {
    ...
    widgets.forEach(function (widget) {
      ...
      widget.scene.userData.lights[0].position.set(
        widget.scene.userData.camera.position.x -
