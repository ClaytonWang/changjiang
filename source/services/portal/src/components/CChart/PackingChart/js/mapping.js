...
var mapItems = function(data, mapping) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    var borderColor = parseInt(properties.border_color, 16);
    var highlightColor = parseInt(properties.highlight_color, 16);
    var item = new Item(parseFloat(properties.x_start),
                        parseFloat(properties.y_start),
                        parseFloat(properties.z_start),
                        width, height, depth, color, borderColor, highlightColor, element);
    items.push(item);
  }
  //console.log(items);
  return items;
